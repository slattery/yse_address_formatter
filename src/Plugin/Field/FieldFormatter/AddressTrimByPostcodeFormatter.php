...
/**
 * Plugin implementation of the 'address_trim_by_postcode' formatter.
 *
 * @FieldFormatter(
 *   id = "address_trim_by_postcode",
 *   label = @Translation("Trim by Postcode"),
 *   field_types = {
 *     "address",
 *   },
 * )
 */
class AddressTrimByPostcodeFormatter extends FormatterBase implements ContainerFactoryPluginInterface {
  ...
  /**
   * Constructs an AddressTrimByPostcodeFormatter object.
   *
  ...
  protected function viewElement(AddressInterface $address, $langcode) {
    ...
    $element = [
      '#theme' => 'address_trim_by_postcode',
  ...
  protected function getValues(AddressInterface $address, AddressFormat $address_format) {
    ...
    $postal_candidate = $values['postalCode'];
    $values['trimTrue'] = in_array($postal_candidate, $postals_to_match);
    // end hack
