...
namespace Drupal\yse_address_formatter\Plugin\Field\FieldFormatter;
...
/**
 * Plugin implementation of the 'address_trim_by_homecountry' formatter.
 *
 * @FieldFormatter(
 *   id = "address_trim_by_homecountry",
 *   label = @Translation("Trim Homecountry off of Addresses"),
 *   field_types = {
 *     "address",
 *   },
 * )
 */
class AddressTrimByHomecountryFormatter extends FormatterBase implements ContainerFactoryPluginInterface {
  ...
  /**
   * Constructs an AddressTrimByHomecountryFormatter object.
   *
  ...
  protected function viewElement(AddressInterface $address, $langcode) {
    ...
    $element = [
      '#theme' => 'address_trim_by_homecountry',
      '#trim' => $values['trimTrue'],
  ...
  protected function getValues(AddressInterface $address, AddressFormat $address_format) {
    ...
    //hack until we build config
    $homecountries_to_match = array("US");
    $postal_candidate = $address->getCountryCode();
    $values['trimTrue'] = in_array($postal_candidate, $homecountries_to_match);
    // end hack
